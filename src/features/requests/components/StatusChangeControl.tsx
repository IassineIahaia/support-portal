...
export function StatusChangeControl({ request }: StatusChangeControlProps) {
  ...
  const [invalidTransitionMsg, setInvalidTransitionMsg] = useState<
    string | null
  >(null);
  ...
  async function handleTransition(nextStatus: ServiceRequestStatus) {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 409) {
        ...
        setInvalidTransitionMsg(
          err.problem.detail ?? "This status transition is not allowed.",
        );
      }
    }
  }
  ...
  return (
    <div>
      ...
      {conflict && (
        <div
          role="alert"
          className="mt-3 bg-tertiary/10 border border-tertiary/30 rounded-standard p-3 flex items-center justify-between gap-4"
        >
          <span className="font-body text-sm text-tertiary">
            This request was updated by someone else. Refresh to see the latest
      ...
      {invalidTransitionMsg && (
        <div
          role="alert"
          className="mt-3 bg-tertiary/10 border border-tertiary/30 rounded-standard p-3"
        >
          <span className="font-body text-sm text-tertiary">
            {invalidTransitionMsg}
          </span>
